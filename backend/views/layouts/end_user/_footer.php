<footer id="colophon" class="site-footer" role="contentinfo">
    <div class="col-full">
        <div class="footer-widgets row-1 col-3 fix">
            <div class="block footer-widget-1">
                <div id="text-3" class="widget widget_text">
                    <span class="gamma widget-title">THÔNG TIN VỀ EIDAI</span>
                    <div class="textwidget">
                        <p>
                            <img loading="lazy" class="alignnone wp-image-124"
                                 src="/img/logo.png" alt="" width="135"
                                 height="43"/>
                            <br/>
                            <a href="https://www.eidai.com">EIDAI Nhật Bản</a>
                            <br/>
                            <a href="http://eidai.com.vn">EIDAI Việt Nam</a>
                        </p>
                        <p>
                            <strong>Địa chỉ: </strong>Dong Van II Industrial Park, Duy Tiên Distric, Hà Nam
                        </p>
                    </div>
                </div>
            </div>
            <div class="block footer-widget-2">
                <div id="text-4" class="widget widget_text">
                    <span class="gamma widget-title">LIÊN HỆ</span>
                    <div class="textwidget">
                        <p>
                            <strong>Công ty TNHH Gỗ và Nội Thất Thăng Long</strong>
                        </p>
                        <p>
                            <strong>Hà Nội Showroom:</strong> LK49, Lô 11, khu dịch vụ Dương Nội, P. Dương Nội, Q. Hà
                            Đông, TP. Hà Nội
                        </p>
                        <p>
                            <strong>Hotline: <em>Mr.Hùng </em>
                            </strong>
                            <a href="tel:0977185377">0977 185 377</a>
                            <a href="tel:0977185377">, </a>
                            <strong>
                                <em>Mr.Bình </em>
                            </strong>
                            <a href="tel:0977185377">0963910684</a>
                        </p>
                        <p>
                            <strong>Hồ Chí Minh Office:</strong> 42/1 đường số 19, khu phố 4, phường Hiệp Bình Chánh,
                            quận Thủ Đức. <br/>
                            <strong>Hotline: <em>Mr.Định</em>
                            </strong>
                            <a href="tel:09863483720">0986 348 372</a>
                        </p>
                        <p>
                            <strong>Email:</strong> sales.eidai@gmail.com
                        </p>
                    </div>
                </div>
            </div>
            <div class="block footer-widget-3">
                <div id="text-6" class="widget widget_text">
                    <span class="gamma widget-title">MẠNG XÃ HỘI</span>
                    <div class="textwidget">
                        <p>
                            <img loading="lazy" class="alignnone wp-image-118"
                                 src="/img/icons8-instagram-100.png" alt=""
                                 width="64" height="64"/>
                            <a href="https://www.facebook.com/sangonhatban/">
                                <img loading="lazy" class="alignnone wp-image-119"
                                     src="/img/icons8-facebook-100.png" alt=""
                                     width="65" height="65"/>
                            </a>
                            <img loading="lazy" class="alignnone wp-image-120"
                                 src="/img/icons8-twitter-100.png" alt=""
                                 width="66" height="66"/>
                            <a href="https://www.youtube.com/channel/UCO6W7tJ1OP27-e99Yl4EDvg/">
                                <img loading="lazy" class="alignnone wp-image-130 size-full"
                                     src="/img/icons8-youtube-64.png" alt=""
                                     width="64" height="64"/>
                            </a>
                        </p>
                        <p>
                            <img loading="lazy" class="wp-image-127 alignleft"
                                 src="/img/an_tin_nhan_Zalo_1-300x118.jpg"
                                 alt="" width="74" height="29"
                                 srcset="/img/an_tin_nhan_Zalo_1-300x118.jpg 300w, /img/an_tin_nhan_Zalo_1-500x196.jpg 500w, /img/an_tin_nhan_Zalo_1-1024x401.jpg 1024w, /img/an_tin_nhan_Zalo_1-768x301.jpg 768w, /img/an_tin_nhan_Zalo_1.jpg 1440w"
                                 sizes="(max-width: 74px) 100vw, 74px"/>TP.HCM: 0986 348 372
                        </p>
                        <p>TP.HN: 0963910684</p>
                        <p>TP. Đà Nẵng: 0934056468</p>
                    </div>
                </div>
            </div>
        </div>
        <!-- .footer-widgets.row-1 -->
        <div class="site-info"> &copy; SÀN GỖ CAO CẤP NHẬT BẢN 2022 <br/>
        </div>
        <!-- .site-info -->
        <div class="storefront-handheld-footer-bar">
            <ul class="columns-3">
                <li class="my-account">
                    <a href="https://eidai.com.vn/my-account/">Tài khoản của tôi</a>
                </li>
                <li class="search">
                    <a href="">Tìm kiếm</a>
                    <div class="site-search">
                        <div class="widget woocommerce widget_product_search">
                            <form role="search" method="get" class="woocommerce-product-search"
                                  action="https://eidai.com.vn/">
                                <label class="screen-reader-text" for="woocommerce-product-search-field-2">Tìm
                                    kiếm:</label>
                                <input type="search" id="woocommerce-product-search-field-2" class="search-field"
                                       placeholder="Tìm sản phẩm&hellip;" value="" name="s"/>
                                <button type="submit" value="Tìm kiếm">Tìm kiếm</button>
                                <input type="hidden" name="post_type" value="product"/>
                            </form>
                        </div>
                    </div>
                </li>
                <li class="cart">
                    <a class="footer-cart-contents" href="https://eidai.com.vn/cart/" title="Xem giỏ hàng của bạn">
                        <span class="count">0</span>
                    </a>
                </li>
            </ul>
        </div>
    </div>
    <!-- .col-full -->
</footer>